...
NotaFiscal.prototype.ImprimeNotaFiscal = function (url) {
...
NotaFiscal.prototype.ToggleVisibility = function (id) {
    if ($("#" + id).css('opacity') === '1') {
...
NotaFiscal.prototype.HideHeader = function () {
    $("#nfHeader").hide(1500);
};
NotaFiscal.prototype.ShowHeader = function () {
    $("#nfHeader").show(1500);
};
NotaFiscal.prototype.UpdateItemQtd = function (nfid, line, qtd) {
    $.post("/ContaPrs/NotaFiscalUpdateItemQtd", { nfid: nfid, line: line, qtd: qtd }, function (data) {});
    UpdateItemTotal(nfid, line);
};
NotaFiscal.prototype.UpdateItemDescricao = function (nfid, line, descricao) {
    $.post("/ContaPrs/NotaFiscalUpdateItemDescricao", { nfid: nfid, line: line, descricao: descricao }, function (data) {});
};
NotaFiscal.prototype.UpdateItemUnitario = function (nfid, line, unitario) {
    $.post("/ContaPrs/NotaFiscalUpdateItemUnitario", { nfid: nfid, line: line, unitario: unitario }, function (data) {});
    UpdateItemTotal(nfid, line);
};
NotaFiscal.prototype.GetNumeroNotaFiscal = function (nfid) {
    $.post("/ContaPrs/NotaFiscalGeraNumero", { nfid: nfid }, function (data) {
        return data;
    });
};
NotaFiscal.prototype.GeraNumeroNotaFiscal = function (nfid) {
    $.post("/ContaPrs/NotaFiscalGeraNumero", { nfid: nfid }, function (data) {
        if (data > 0) {
...
NotaFiscal.prototype.AtualizaNumeroNotaFiscalUI = function (data) {
    var numerocontainer = document.getElementById("NumeroNotaContainerContent");
...
UpdateItemTotal = function (nfid, line) {
    ...
    try {
        ...
        if (qtd > 0) document.getElementById(target).value = isNaN(qtd * unt) ? '' : Nfmt.ToMoney(qtd * unt);
...
RefreshAvailableActions = function () {
    Msg();
...
PodeGerarNumero = function () {
    return parseInt(document.getElementById("NumeroNf").innerText) === 0 && TotalNota > 0;
};
PodeImprimir = function () {
...
EscondeBotaoImprimir = function () {
    var e = document.getElementById("BotaoImprimir");
    e.classList.add('noshow');
};
MostraBotaoImprimir = function () {
    var e = document.getElementById("BotaoImprimir");
    e.classList.remove('noshow');
};
MostraGerarNumeroNota = function () {
    document.getElementById("GerarNumeroNota").classList.remove('hidden');
};
EscondeGerarNumeroNota = function () {
    document.getElementById("GerarNumeroNota").classList.add('hidden');
};
MostrarNumeroNota = function (forPrint) {
...
MostrarEmissao = function (color) {
    var em = document.getElementById("emissao");

    em.innerText = " DATA DE EMISSÃO : " + Hoje();
    em.style.color = color;
};
EscondeNumeroNota = function () {
    document.getElementById("NumeroNfContainer").classList.add('hidden');
};
AtualizaNotaFiscalUi = function () {
